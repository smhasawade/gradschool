...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        exists = False
        '''for n in self.nodes:
            if str(n[0]) == id or str(n[1]) == name:
                exists = True'''
        exists = (str(id), str(name)) in self.nodes
        if exists == False:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        exists = False
        '''for e in self.edges:
            if str(e[0]) == source and str(e[1]) == target:
                exists = True
            elif str(e[1]) == source and str(e[0]) == target:
                exists = True'''
        exists = (str(source), str(target)) in self.edges or (str(target), str(source)) in self.edges
        if exists == False:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        max_deg = {}
        for e in self.edges:
            if e[0] not in max_deg:
                max_deg[e[0]] = 1
            else:
                max_deg[e[0]] += 1

            if e[1] not in max_deg:
                max_deg[e[1]] = 1
            else:
                max_deg[e[1]] += 1
        
        max_val = max(max_deg.values())
        final = {}
        for key, value in max_deg.items():
            if value == max_val:
                final[key] = value
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        connection.request("GET", "/3/movie/"+movie_id+"/credits?api_key="+self.api_key+"&callback=test")
        response = connection.getresponse()
        results = response.read().decode('utf-8')
        data = results.replace("test(","").replace(")","")
        json_text = json.loads(data)
        connection.close()
        if "cast" in json_text:
            cast = json_text["cast"]
        else:
            cast = []
            print(movie_id)
        ...
        if exclude_ids is not None:
            cast[:] = [d for d in cast if int(d.get('id')) not in exclude_ids]
        for c in cast:
            order = int(c['order'])
            id = int(c['id'])
            ...
            if include == True:
                final.append({'id':c['id'],'character':c['character'],'credit_id':c['credit_id'],'name':c['name'].replace(","," "),'order': c['order']})
            include = True
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        """
        connection = http.client.HTTPSConnection("api.themoviedb.org")
        connection.request("GET", "/3/person/"+person_id+"/movie_credits?api_key="+self.api_key+"&callback=test")
        response = connection.getresponse()
        results = response.read().decode('utf-8')
        data = results.replace("test(","").replace(")","")
        json_text = json.loads(data)
        ...
        final = []
        for c in cast:
            vot_avg = float(c['vote_average'])
            if vote_avg_threshold is not None:               
                if vot_avg >= vote_avg_threshold:
                    include = True
                else:
                    include = False
            else:
            ...
            if include == True:
                final.append({'id': c['id'],'title':c['title'],'vote_average':c['vote_average']})
            include = True

        return final
        
def build_base_graph(person_id:str, person_name:str, key:str):
    graph = Graph()
    graph.add_node(id=person_id, name=person_name)
    tmdb_api_utils = TMDBAPIUtils(api_key=key)
    exclude =[int(person_id)]
    credits = tmdb_api_utils.get_movie_credits_for_person(person_id, 8.0)    
    for cred in credits:
        cast = tmdb_api_utils.get_movie_cast(str(cred['id']), 3, exclude)
        for c in cast:
            exclude.append(int(c['id']))
            graph.add_node(id=str(c['id']), name=str(c['name']))
            graph.add_edge(str(person_name), str(c['name']))
    ...
    for n in graph.nodes:
        new_cred =  tmdb_api_utils.get_movie_credits_for_person(n[0], 8.0)
        credits += new_cred    
        names = names  + ([n[1]] * len(new_cred))
    i = 0
    for cred in credits:
        cast = tmdb_api_utils.get_movie_cast(str(cred['id']), 3, exclude) 
        for c in cast:
            exclude.append(int(c['id']))
            nodes_added.append((str(c['id']),str(c['name'])))
            graph.add_node(id=str(c['id']), name=str(c['name']))
            graph.add_edge(names[i], str(c['name']))
        i=i+1
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#a53fa719-03fe-45cb-b51a-01278ff2460a"
...
if __name__ == "__main__":
    #my test
    test = Graph()
    test.add_node('1234', 'Robert Singer')
    test.add_node('1234', 'Robert Singer')
    test.add_node('2345', 'Eric Kripke')
    test.add_node('3456', 'Harry Potter')
    test.add_edge('1234', '2345')
    test.add_edge('1234', '3456')
    test.add_edge('2345', '3456')
    test.print_nodes() 
    test.print_edges()
    print(test.max_degree_nodes())

    shruti_tmdb =TMDBAPIUtils("d5a7e9f8c8bd364b4f6e20747b0caa0a")
    shruti_tmdb.get_movie_cast('508943', 5)
    shruti_tmdb.get_movie_cast('550', 10, [8,9])
    shruti_tmdb.get_movie_credits_for_person('819') 
    shruti_tmdb.get_movie_credits_for_person('4785', 5.5) 
    ...
    graph = build_base_graph('2975', 'Laurence Fishburne', "d5a7e9f8c8bd364b4f6e20747b0caa0a")
    graph.print_nodes()
    graph.print_edges()
    graph.write_edges_file()
    graph.write_nodes_file()
