...
class HW2_sql():
    ...
    # GTusername [0 points]
    def GTusername(self):
        gt_username = "smhasawade6"
        return gt_username
    
    # Part a.i Create Tables [2 points]
    def part_ai_1(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_ai_1_sql = "CREATE TABLE movies(id INTEGER NOT NULL,title TEXT NOT NULL,score REAL NOT NULL);"
        ######################################################################
    ...
    def part_aii_1(self,connection,path):
        ...
        for r in rows:
           connection.execute("INSERT INTO movies (id, title, score) VALUES ("+r[0]+","+'"'+r[1].replace('"','')+'"'+","+r[2]+");")
           connection.commit()
    ...
    def part_aii_2(self,connection, path):
        ...
        rows = csv.reader(a_file)
        for r in rows:
           connection.execute("INSERT INTO movie_cast(movie_id, cast_id, cast_name, birthday, popularity) VALUES ("+r[0]+","+r[1]+","+'"'+r[2].replace('"','')+'"'+","+'"'+r[3].replace('"','')+'"'+","+r[4]+");")
           connection.commit()
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_sql = "CREATE TABLE cast_bio (cast_id INTEGER PRIMARY KEY,cast_name TEXT NOT NULL,birthday TEXT NOT NULL,popularity REAL NOT NULL);"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT printf(\"%.2f\", 100.0 * CAST(count(id)AS FLOAT)/(SELECT count(id) FROM movies)) FROM movies WHERE score > 50 AND title LIKE '%war%';"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name, count(cast_id) FROM movie_cast WHERE CAST(popularity AS REAL)> 10 GROUP BY cast_id ORDER by count(cast_id) DESC, cast_name ASC LIMIT 5;"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT m.title, printf(\"%.2f\",m.score), count(cast_id) FROM movies m, movie_cast c WHERE c.movie_id=m.id GROUP BY movie_id ORDER by score DESC, count(cast_id) ASC, title DESC LIMIT 5;"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT c.cast_id, c.cast_name, printf(\"%.2f\",avg(score)) FROM movies m, movie_cast c WHERE c.movie_id=m.id AND CAST(m.score AS REAL)>25 GROUP BY cast_id HAVING count(movie_id) > 2 ORDER by avg(m.score) DESC, c.cast_name ASC LIMIT 10;"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "CREATE VIEW good_collaboration AS SELECT c1.cast_id AS cast_member_id1, c2.cast_id AS cast_member_id2, count(*) AS movie_count, printf(\"%.2f\",avg(m.score)) AS average_movie_score FROM movies m INNER JOIN movie_cast c1 ON c1.movie_id=m.id INNER JOIN movie_cast c2 ON c1.movie_id=c2.movie_id WHERE c1.cast_id <> c2.cast_id AND c1.cast_id < c2.cast_id GROUP BY c1.cast_id, c2.cast_id HAVING avg(m.score)>=40 AND count(*) > 2;"
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "WITH data AS(SELECT cast_member_id1 AS cast_id, m.cast_name, printf(\"%.2f\",avg(average_movie_score))AS collaboration_score FROM good_collaboration INNER JOIN movie_cast m ON cast_member_id1 = m.cast_id GROUP BY cast_member_id1 UNION SELECT cast_member_id2 AS cast_id, m.cast_name, printf(\"%.2f\",avg(average_movie_score))AS collaboration_score FROM good_collaboration INNER JOIN movie_cast m ON cast_member_id2 = m.cast_id GROUP BY cast_member_id2) SELECT * FROM data ORDER BY collaboration_score DESC,cast_name ASC LIMIT 5;"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        a_file = open(path, encoding='utf-8-sig')
        rows = csv.reader(a_file)
        for r in rows:
           connection.execute("INSERT INTO movie_overview(id, overview) VALUES ("+r[0]+","+'"'+r[1].replace('"','')+'"'+");")
           connection.commit()
        ######################################################################
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = "SELECT count(*) FROM movie_overview WHERE movie_overview.overview MATCH 'fight';"
        ######################################################################
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = "SELECT count(*) FROM movie_overview WHERE movie_overview.overview MATCH 'space NEAR/5 program';"
        ######################################################################
